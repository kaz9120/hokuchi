...
かつてここにあった `stage_margin` (レターボックス帯) は 0.3.0 で撤去しました (ADR-0014)。余白の配分は構図の知識であり、テーマではなくレンダラが持ちます。
...
| `source` | 文字列 | 任意 | ポストの URL。handout に載るほか、SPA では実埋め込みの導出元になる |

静的出力 (shot / handout) はカード描画です。SPA 出力では `source` を持つ post に限り、X の埋め込み (oEmbed) をカードに重ねて試みます。ネットワークが無い・ポストが消えている場合はカードがそのまま見えます (漸進的強化、ADR-0017)。
...
| フィールド | 型 | 必須 | 意味 |
|-----------|----|----|------|
| `url` | 文字列 | 必須 | 紹介先の URL。QR と OGP 解決の導出元 |
| `title` | 文字列 | 任意 | 記事タイトル。未指定なら OGP から解決 (ADR-0017) |
| `description` | 文字列 | 任意 | 1 行の紹介文。未指定なら OGP から解決 |
| `image` | 文字列 | 任意 | OGP 画像のローカルパス。未指定なら OGP から解決 |

`title` / `description` / `image` が未指定のとき、render はページの OGP メタデータを取得して補い、結果をデッキの `assets/ogp/` にキャッシュします (存在すれば再取得しない。オフライン再レンダリングと final 凍結に耐える)。手書きの指定は常に優先されます。取得に失敗しても render は止まらず、テキストのみのカードに落ちます (ADR-0017)。カードの標準構成は OGP 画像 + タイトル + URL 全体 + QR です。
...
レンダラはスキーマ外の実装です。本章が規定するのは「正しさの床」だけです (ADR-0014)。構図 — 比率・余白の配分・光学中心・図形の幾何 — はレンダラ内部の実装詳細であり、本仕様には書きません。美しさの改善は本章の改版を必要としません。

### 8.1 キャンバス

キャンバスは 1280×720 (16:9) の固定とします。キャンバスの内側をどう分割するか (マージン・見出し帯・主役の箱・余白) はレンダラが決めます。role はレイアウトの意味を区別します (ADR-0008-6): opener / closer とフルブリード画像は縁まで使ってよく、content 系は可読の余白を保ちます。具体的な余白量は規定しません。
